...
def create_arg_parser():
    ...
    p.add_argument('--num_train_epochs', type=int, default=2, help='Number of training epochs.')
    ...
    p.add_argument('--max_length', type=int, default=64, help='Maximum sequence length for tokenizer.')
    p.add_argument('--early_stopping_patience', type=int, default=3, help='Patience for early stopping.')
    p.add_argument('--seed', type=int, default=42, help='Random seed for reproducibility.')
    p.add_argument('--hashtag_handling', type=str, default='none', choices=['none', 'remove', 'integrate'],
                        help='How to handle hashtags: "none", "remove", or "integrate".')
    return p.parse_args()

def read_corpus(corpus_file, hashtag_handling='none'):
    """
    Reads a two-column TSV file (text, label) for the offensive language task,
    with an option for hashtag preprocessing.
    """
    documents, labels = [], []
    with open(corpus_file, encoding='utf-8') as f:
        for line in f:
            ...
            if len(parts) == 2:
                ...
                elif hashtag_handling == 'integrate':
                    text = integrate_hashtags(text)
...
def main():
    ...
    X_train, Y_train = read_corpus(args.train_file, args.hashtag_handling)
    X_dev, Y_dev = read_corpus(args.dev_file, args.hashtag_handling)
    ...
    if args.test_file:
        ...
        X_test, Y_test = read_corpus(args.test_file, args.hashtag_handling)
